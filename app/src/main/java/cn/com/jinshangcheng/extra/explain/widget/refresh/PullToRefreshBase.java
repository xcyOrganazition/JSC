...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    protected final void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            mScrollingWhileRefreshingEnabled = bundle
                    .getBoolean(STATE_SCROLLING_REFRESHING_ENABLED, false);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // Styleables from XML
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.cst_platform_PullToRefresh);

        if (a.hasValue(R.styleable.cst_platform_PullToRefresh_ptrMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.cst_platform_PullToRefresh_ptrMode, 0));
        }

        if (a.hasValue(R.styleable.cst_platform_PullToRefresh_ptrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.cst_platform_PullToRefresh_ptrAnimationStyle, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.cst_platform_PullToRefresh_ptrRefreshableViewBackground)) {
            Drawable background = a
                    .getDrawable(R.styleable.cst_platform_PullToRefresh_ptrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(R.styleable.cst_platform_PullToRefresh_ptrAdapterViewBackground)) {
            Drawable background = a.getDrawable(R.styleable.cst_platform_PullToRefresh_ptrAdapterViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(R.styleable.cst_platform_PullToRefresh_ptrOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.cst_platform_PullToRefresh_ptrOverScroll, true);
        }

        if (a.hasValue(R.styleable.cst_platform_PullToRefresh_ptrScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    R.styleable.cst_platform_PullToRefresh_ptrScrollingWhileRefreshingEnabled, false);
        }
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            case PULL_FROM_END:
                newScrollValue = Math
                        .round(Math.max(initialMotionValue - lastMotionValue, 0) / FRICTION);
                itemDimension = getFooterSize();
                break;
            case PULL_FROM_START:
            default:
                newScrollValue = Math
                        .round(Math.min(initialMotionValue - lastMotionValue, 0) / FRICTION);
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            } else if (mState == State.PULL_TO_REFRESH && itemDimension < Math
                    .abs(newScrollValue)) {
    ...
    public static interface OnPullEventListener<V extends View> {
         ...
         */
        public void onPullEvent(final PullToRefreshBase<V> refreshView, State state,
                                Mode direction);
